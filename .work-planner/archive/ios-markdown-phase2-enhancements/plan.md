...
### S8 — 列表元信息显示（对齐系统「文件」App）
- Goal：首页/子目录列表每行在名称下显示一行元信息，向系统「文件」App 看齐。
- Sub-steps：
  - S8.1 `DocumentNode` 增 `fileSize`/`childCount` 字段 + `metadataText`（修改时间 · 大小/子项数），格式化用静态 Formatter 复用。
  - S8.2 `FileStore.contents(of:)` 加载时读入文件大小；文件夹用浅层计数 `visibleChildCount`（不递归、排除 Inbox）算子项数。
  - S8.3 `FileBrowserView` 行改为「图标 + 名称 + 副标题」两行布局。
- Verify：文件显示「时间 · KB」、文件夹显示「时间 · N 项」；时间今天显示时刻、昨天/前天/更早合理；子项数与进入后可见项一致。
- 归属：属 M1 体验增强的补充（用户 2026-07-12 追加）。
