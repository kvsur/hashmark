...
- Sub-steps：
  - S8.1 注册**付费 Apple Developer Program**（99 美元/年）。
  - S8.2 App ID / Bundle Identifier、证书与 provisioning（Xcode 自动签名优先）。
  - S8.3 隐私清单（Privacy Manifest / `PrivacyInfo.xcprivacy`）——本 App 不联网、不收集数据，如实声明。
  - S8.4 App Store Connect 建条目：名称、描述、分类、截图（各机型尺寸）。
  - S8.5 Archive → 上传 → TestFlight 内测安装。
  - S8.6 提交审核。
- Verify：TestFlight 能装到真机（里程碑）；提交审核成功。

### S9 —（后续）iCloud 同步（MVP 后再做）
- Goal：文档改存 iCloud 容器，多设备自动同步。
- Sub-steps（占位，届时细化）：
  - S9.1 开启 iCloud Documents entitlement + ubiquity container。
  - S9.2 `FileStore` 迁移到 iCloud 容器路径，处理下载/冲突状态。
  - S9.3 本地已有文档的迁移策略。
